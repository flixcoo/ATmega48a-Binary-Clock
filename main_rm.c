...
int main() {
    ...
    while (1) {
        ...
        if ((debounce_button(BUTTON3)) && (debounce_button(BUTTON2))) { // Taster 2 + 3 werden gedrueckt
            cycle_dimming_steps();
            _delay_ms(100); // Entprellung hoeher, da zwei Taster gedrueckt werden
        } else if (debounce_button(BUTTON3) && !accuracy_test) {
            currentTime.minutes = (currentTime.minutes + 1) % 60; // Erhoehe Minuten um 1, maximal bis 60
            _delay_ms(50); // Entprellung
        } else if (debounce_button(BUTTON2) && !accuracy_test) {
            currentTime.hours = (currentTime.hours + 1) % 24; // Erhoehe Stunden um 1, maximal bis 24
            _delay_ms(50);  // Entprellung
        } else if (debounce_button(BUTTON1) && !accuracy_test) {
...
void init_clock(void) {
    ...
    BUTTON_DDR  &= ~((1 << PD5) | (1 << PD6) | (1 << PD7)); // Taster an PD5, PD6 und PD7 als Eingang
    BUTTON_PORT |= (BUTTON1 | BUTTON2 | BUTTON3);          // Pull-up Widerstände der Taster aktivieren
...
void display_time() {
    ...
    for (uint8_t i = 0; i < 5; i++) {           // Durch die Fuenf Stunden-LEDs iterieren
        if (currentTime.hours & (1 << i)) {     // Die Bits der aktuellen Zeit werden der Reihe nach ausgelesen
            HOUR_LEDS_PORT |= (1 << (i)); // ... und auf die Stunden LEDs geschrieben
        }
    }
    ...
    for (uint8_t i = 0; i < 6; i++) {           // Durch die Sechs Minuten-LEDs iterieren
        if (currentTime.minutes & (1 << i)) {   // Die Bits der aktuellen Zeit werden der Reihe nach ausgelesen
            MINUTE_LEDS_PORT |= (1 << i);   // ... und auf die Stunden LEDs geschrieben
        }
    }
}
...
void all_leds_on() {
    ...
    PORTD |= 0x1F; //11111000 - Stunden-LEDs
}
